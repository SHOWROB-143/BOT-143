...
usages: "facebook cover photo make by TRA-ARIFVAU",
cooldowns: 0,
dependencies: {
    "fs-extra": "",
    "request": "",
...
if ((this.config.credits) != "TRA-ARIFVAU") { return api.sendMessage(`[ WARNING ] -itz me TRA  credits modules ${this.config.name} was changed to ${this.config.credits} bởi ADMINBOT ${global.config.BOTNAME} 😐 Stop now!!!`, event.threadID, event.messageID)}
 const request = require('request');
  ...
  else return api.sendMessage(`BOT BY SHOWROB\n\n♻️আপনি আপনার  নাম বাছাই করেছেন: ${args.join(" ").toUpperCase()}\n\n[আপনি কি continue করতে চান?]`,event.threadID, (err, info) => {
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  switch (handleReply.type) {
    case "tenphu": {
      ...
      return api.sendMessage(`♻️আপনার সহজে fb  এর কিছু নাম দিয়া হলো:
    ...
    case "create": {
      ...
      let background = (
        await axios.get(encodeURI(`https://i.postimg.cc/rps2V8qB/20230319-221959.jpg`), {
          responseType: "arraybuffer",
        })
      ).data;
      let hieuung = (
        await axios.get(encodeURI(`https://i.postimg.cc/BbcxwfP6/20230319-220929.png`), {
          responseType: "arraybuffer",
        })
      ).data;
      ...
      ctx.font = "48px UTMAvoBold";
      ctx.fillText(name, 460, 120);
      ctx.font = "48px UTMAvoBold";
      ctx.fillStyle = "#fff";
      ctx.textAlign = "right";
      
      ctx.font = "28px UTMAvoBold";
      ctx.fillStyle = "#fff";
      ctx.textAlign = "start";
      ctx.fillText(phoneNumber, 533, 210);
      ctx.fillText(email, 533, 280);
      ctx.fillText(address, 533, 348);
      ...
      ctx.drawImage(baseAva, 42, 112, 290, 290);
